...
import { SupplierStatusButton } from "@/components/parties/status-button";
...
export default async function SuppliersPage({ searchParams }: Props) {
    ...
    const result = await partyRepository.listSuppliers(params);
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">{result.total} pemasok</p>
                </div>
                {hasPermission(ctx.role, "supplier.create") && <PartyDialogButton variant="supplier" />}
            </div>

            <form action="/suppliers" method="get" className="flex flex-wrap items-end gap-2 rounded-lg border p-4">
                            ...
                            {result.items.map((s) => (
                                <tr key={s.id} className="border-b last:border-0 hover:bg-muted/30">
                                    ...
                                    <td className="whitespace-nowrap p-3 text-right">
                                        {hasPermission(ctx.role, "supplier.update") && (
                                            <span className="mr-2">
                                                <PartyDialogButton variant="supplier" initial={{
                                                    id: s.id, name: s.name, phone: s.phone ?? "",
                                                    email: s.email ?? "", address: s.address ?? "",
                                                }} />
                                            </span>
                                        )}
                                        {hasPermission(ctx.role, "supplier.delete") && (
                                            <SupplierStatusButton id={s.id} active={s.status === "ACTIVE"} />
                                        )}
                                    </td>
                                </tr>
                            ))}
